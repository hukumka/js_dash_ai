...
exports.play = function*(screen){
	...
	while(true){
		...
		if(test != null){
			...
			old_world.evalute(old_world.next_command)
        ...
        old_world.next_command = command
...
function check_if_safe(world, state, move){
	...
	let {alive, good} = ban_dead_moves(world, state)
...
function improved_determine_action(world, state){
	...
	if(but_kill_move !== false){
	    ...
	    if(world.map[y][x] === '*' || (y>0 && world.map[y-1][x] === '@')){
			console.error("DIAMOND")
		}
	...
	while(!moves.empty()){
		...
		if('*@'.includes(world.map[y][x])){
			...
			if(state.diamond != null && state.diamond.x==x && state.diamond.y === y){
				if(world.streak_end_frame - world.frame < distance - 2){
					state.diamond = null
					break
				}else{
					return initial_move;
				...
				if(diamond_map[y][x] != null && !dist_graph.joints[diamond_map[y][x]] && world.streak_end_frame - world.frame >= distance - 2){
					state.diamond = {x, y}
					return initial_move
				}
			}
            ...
            if(x===2 && y===5){
			    console.error(loose1, loose2, loose3)
            }
            if(world.map[y][x] === ':'){
...
function free_all_butterflies(world, state) {
    ...
    while (!moves.empty()) {
        ...
        visited[y][x] = priority;
        if (is_butterfly_neighbor(state.routes, {x, y}, priority)) {
            let is_free = false
...
function kill_butterfly(world, state){
	let routes = get_butterfly_routes(world, 30)
	let rc = 0
	for(let r of routes){
		rc++
	}
...
function search(world, state, weight_func, searh_func, find_all){
	...
	for(let move of state.good_moves){
		let {x, y} = move_pos(state.pos, move)
		queue.push({x, y, distance: 0, move}, 0)
	}
	...
	while(!queue.empty()){
		...
		if(visited[y][x] > priority){
			...
			visited[y][x] = priority
			let w = weight_func(world, x, y)
            if(searh_func(world, x, y)){
				...
				for(let j=2; j*2<dr.length && j<=i; ++j){
					if(dr[j*2] === r[i].x && dr[j*2+1]+1 === r[i].y){
						...
						if(offset > distance_map[dr[1]][dr[0]].dist){
                            drop.offset = offset
                            console.error(drop, offset)
                            if(!is_butterfly_route_intersect(routes, {x: dr[0], y: dr[1]}, offset)) {
                                let drop_to_push = {offset, route: drop.route, x: drop.x, y: drop.y}
...
function execute_killing_drop(world, state){
	...
	if(state.exec_drop !== undefined && state.exec_drop.x === state.pos.x && state.exec_drop.y+1 === state.pos.y){
		...
		if(is_butterfly_route_intersect(state.routes, {x:d.x, y:d.y+1}, d.offset)){
	...
	for(let d of drops){
		if(d.offset > dm[d.y+1][d.x].dist + 2){
		    if(is_butterfly_route_intersect(state.routes, {x:d.x, y:d.y+1}, d.offset)){
...
function is_butterfly_route_intersect(routes, pos, max_time){
	for(let i=0; i<max_time+2; ++i){
		if(is_butterfly_neighbor(routes, pos, i)){
			return true
		}
	}
	return false
